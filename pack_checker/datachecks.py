...
def _locations_get_section_ids(data: t.Any, start: t.Optional[str] = None) -> t.Generator[str, None, None]:
    assert isinstance(data, list), "locations data not a list"  # noqa: S101 type checking, would fail below
    for location in data:
        assert isinstance(location, dict), "location not a dict"  # noqa: S101 type checking, would fail below
        ...
        for section in sections:
            assert isinstance(section, dict), "section not a dict"  # noqa: S101 type checking, would fail below
